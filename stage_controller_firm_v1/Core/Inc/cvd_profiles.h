...
/* v1: Enable を SPI(NET_IN の ENABLE ビット)経由にする。EN ピンは N.C.。
 *   - setting: ENABLE ソースを SPI に（ENABLE_SRC_SPI）
 *   - net_in : SD_ON に加え ENABLE ビットを立てて励磁ON */
#define PG413_SETTING                                      CVD_SETTING_ENABLE_SPI_PLS_DIR
#define PG413_NET_IN                                       (CVD_NET_IN_SD_ON_FILTER_OFF | CVD_NET_IN_ENABLE)
