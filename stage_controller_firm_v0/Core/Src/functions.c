 ...
 * axis 1 (Y): TIM3 CH4 (PC9),   APB1 (140 MHz), PSC=139 → 1 MHz counter  [RCR 無, 当面未使用]
 * axis 2 (Z): TIM8 CH2 (PC7),   APB2 (280 MHz), PSC=279 → 1 MHz counter  [RCR 有]
 *
 * Y↔Z のタイマを入替え、運転する X/Z を RCR 付きタイマ(TIM1/TIM8)に載せた。
 * 巡航は RCR でパルス計数するため、RCR 無しの TIM3(Y) は当面運転しない。
 */
static TIM_HandleTypeDef * const k_tim[MOTOR_MAX] = { &htim1, &htim3, &htim8 };
...
/* カスケード送信（マスター→スレーブ）。下部で実装。0=成功, 非0=失敗 */
