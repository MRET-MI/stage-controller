...
/* CW(プラス端)ハードウェアリミットの有効/無効。
 *   1 = 有効（通常移動でCW端センサ作動→即停止＋開始前プリチェック）
 *   0 = 無効（暫定: CWセンサ出力が正しく出ないため無効化）
 * CCW(マイナス端)は常に有効。ホーミングのCW過走検出には影響しない。*/
#define LIMIT_CW_ENABLE      0
...
/* 駆動フェーズ: 加速/減速=毎パルス可変ARRのDMA(RCR=0), 巡航=一定ARR連続運転+RCR計数(DMAなし) */
typedef enum {
    PH_IDLE   = 0,
    PH_ACCEL  = 1,
    PH_CRUISE = 2,
...
typedef enum {
    ...
    HM_SLOW_SEEK = 3,   /* CCWへ低速接近（CCW limit作動→原点基準確定）*/
    HM_OFFSET    = 4,   /* 原点(0)へ CW オフセット移動（HOME_OFS>0 のとき）*/
    HM_DONE      = 5,
    HM_ERROR     = 6,
} HomingState_t;
...
typedef struct {
    ...
    uint32_t                seg_issued;   /* pulses issued in current ramp phase (accel/decel; DMAブロック単位) */
    volatile uint32_t       cruise_remain;/* cruise pulses left to count via RCR */
    volatile uint8_t        stop_req;     /* 巡航中の手動停止要求: 現チャンク完了時(UEV)に停止し誤差ゼロ化 */
} MotorCtrl_t;
...
/* 1行のコマンド（; 区切り可, @<box> は含めない）を実行し応答文字列を生成 */
void RunCommandLine(const char *line, char *resp_out, size_t n);

/* カスケード接続（コントローラ間 SPI, hspi1）
 *   Cascade_Init: ブート時に役割へ応じ SPI1 を設定（main.c から MX_SPI1_Init 後に呼ぶ）
 *   Cascade_Poll: スレーブ機の主ループから呼び, 受信コマンドを実行して応答を用意 */
...
extern uint32_t tim1_psc, tim6_psc, tim8_psc, tim15_psc;
extern uint32_t tim1_clk, tim6_clk, tim8_clk, tim15_clk;
...
extern uint8_t  P_is_master, P_num_boxes;   /* カスケード役割・台数 */
